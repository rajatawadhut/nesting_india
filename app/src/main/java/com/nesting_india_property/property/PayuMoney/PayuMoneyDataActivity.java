...
public class PayuMoneyDataActivity extends AppCompatActivity {
    ...
    private void payuMoney(CheckSumData response) {
        ...
        try {
            ...
            PayUmoneyFlowManager.startPayUMoneyFlow(mPaymentParams, PayuMoneyDataActivity.this, R.style.AppTheme_default, true);

        } catch (Exception e) {
            // some exception occurred
            e.printStackTrace();
            Log.d(TAG, "payuMoney: " + e.getMessage());
            Toast.makeText(PayuMoneyDataActivity.this, e.getMessage(), Toast.LENGTH_LONG).show();
        }
    }
